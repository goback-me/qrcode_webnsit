      ...
      <div className="container py-12">
        <div className="grid md:grid-cols-4 gap-8 items-start">
          <div className="md:col-span-2">
            <h5 className="text-2xl font-extrabold text-gray-900 mb-3">GEN QR</h5>
            <p className="muted-text mb-4 max-w-md">{content.footer.about}</p>
          ...
          <div>
            ...
            <ul className="space-y-2">
              {content.footer.quickLinks.map((link: any, index: number) => (
                <li key={index}>
                  <Link href={link.url} className="link-accent muted-text hover:underline">
                    {link.title}
                  </Link>
                </li>
              ))}
          ...
          <div>
            <h6 className="font-semibold text-gray-900 mb-4">Contact</h6>
            {content.footer.contact.split('\n').map((line: string, index: number) => (
              <p key={index} className="muted-text mb-2">{line}</p>
            ))}
          </div>
        </div>

        <div className="border-t border-gray-200 pt-8 mt-8">
          <p className="text-center muted-text text-sm">© 2025 GENQR. All rights reserved.</p>
        </div>
      </div>
    </footer>
  );
